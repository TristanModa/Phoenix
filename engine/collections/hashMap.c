...
HashMap* HashMap_create(const size_t itemSize, const ItemDestructorFn itemDestructor) {
	...
	hashTableConfig.mem_alloc = Memory_malloc;
	hashTableConfig.mem_calloc = Memory_calloc;
	hashTableConfig.mem_free = Memory_free;
...
KeyValuePair* HashMap_insertItem(HashMap* hashMap, const char* key, const void* item) {
	...
	KeyValuePair* keyValuePair = Memory_malloc(sizeof(*keyValuePair));
	char* keyCopy = Memory_malloc(keyLen);
	...
	if (!keyValuePair || !keyCopy || !itemCopy) {
		...
		if (keyValuePair) { Memory_free(keyValuePair); }
		if (keyCopy) { Memory_free(keyCopy); }
		if (itemCopy) { Memory_free(itemCopy); }
		return nullptr;
	}
...
void destroyKeyValuePair(const HashMap* hashMap, KeyValuePair* keyValuePair) {
	...
	Memory_free(keyValuePair->value);
	Memory_free((char*)keyValuePair->key);
	Memory_free(keyValuePair);
}